# Third-party notices

ClearBlock includes the following third-party components and data:

- eyeo WebExtension Ad-Filtering Solution 4.1.0 — GPL-3.0-only.
- EasyList — GPL-3.0-or-later and CC BY-SA 3.0.
- EasyPrivacy — GPL-3.0-or-later and CC BY-SA 3.0.
- EasyList China — GPL-3.0-or-later and CC BY-SA 3.0.
- webextension-polyfill — MPL-2.0.

Build-only dependencies and their license metadata are recorded in
`package-lock.json`. Every generated extension package includes the complete
GPL-3.0 license text in `COPYING`.
